...
class Config(object):
    ...
    def __init__(self):
        ...
        self.lib.getTripleList.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
    ...
    def run(self):
        # init scalars and load content if already existing
        loss_fn = os.path.join(os.path.dirname(self.exportName), "loss.csv") if self.exportName != None else None
        series_loss = pd.read_csv(loss_fn, squeeze=True, index_col=0, encoding="utf-8") \
                if loss_fn != None and os.path.exists(loss_fn) else pd.Series()
        
        # get current epoch
        current_epoch = series_loss.size
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                for times in range(current_epoch, self.train_times):
                    ...
                    # log scalars
                    series_loss = series_loss.append(pd.Series([res], index=[times]))
                    ...
                    if self.export_steps != 0 and times % self.export_steps == 0:
                        ...
                        if self.exportName != None:
                            ...
                            series_loss.to_csv(loss_fn, index=True, encoding="utf-8")
